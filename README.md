...
`placeholder("LABEL")` creates an empty media slot; passing a URL to it only changes the fallback label. For a real image in `media.ts`, use `image("https://.../photo.png", "Descriptive alt text", "LABEL")`. For a video, use a `MediaAsset` with `src`, `type: "video"`, and a fallback `placeholder` label. Restart the local Next.js server after adding a new external hostname so its image allowlist is rebuilt. `.glb` models are not supported by the image/video `Media` component.
